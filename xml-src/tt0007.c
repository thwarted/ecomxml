...
int tt0007_lt_process(request_rec *r, struct global_struct *gbp, char *stdout_buffer)
{
    ...
    // Fill the buffers
        ...
        get_tag_data("CUST_EDP", gbp->sendbuf0007->cust_edp,gbp,stdout_buffer);
...
int tt0007_CatSendStr(struct global_struct *gbp, char *sz_sendbufcat, tt0007_st_send *ptr_sendbuf)
{
    gbp->j = sprintf(sz_sendbufcat,
            "%-4.4s%-4.4s%-2.2s%-2.2s%-16.16s%-16.16s%-25.25s%-8.8s%-3.3s%-9.9s",
...
*/
    ...
    memset(ptr_recvbuf->page_no, '\0', tt0007_LEVEL_LEN + 1);
    memcpy(ptr_recvbuf->page_no, sz_recvbufcat + gbp->count, tt0007_LEVEL_LEN);
    ap_rprintf(r,"		<PAGE_NO>%s</PAGE_NO>\n", handle_special_chars(gbp,ptr_recvbuf->page_no));
    gbp->count += tt0007_LEVEL_LEN;

    memset(ptr_recvbuf->tot_levels, '\0', tt0007_LEVEL_LEN + 1);
    memcpy(ptr_recvbuf->tot_levels, sz_recvbufcat + gbp->count, tt0007_LEVEL_LEN);
    ...
    // Build data from array
        ...
        gbp->l = atoi(ptr_recvbuf->num_levels);

        if(gbp->l > (int)tt0007_LEVEL_COUNT)
                gbp->l = (int)tt0007_LEVEL_COUNT;
                
        for( gbp->i = 0; gbp->i < gbp->l; gbp->i++) 
        {
                ap_rprintf(r,"		<ORDER_DETAILS>\n");

                memset(ptr_recvbuf->level[gbp->i].addr_1, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_1, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE1>%s</SHIP_TO_ADDR_LINE1>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_1));                   
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].addr_2, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_2, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE2>%s</SHIP_TO_ADDR_LINE2>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_2));
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].addr_3, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_3, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE3>%s</SHIP_TO_ADDR_LINE3>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_3));
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].addr_4, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_4, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE4>%s</SHIP_TO_ADDR_LINE4>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_4));
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].addr_5, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_5, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE5>%s</SHIP_TO_ADDR_LINE5>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_5));
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].addr_6, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_6, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE6>%s</SHIP_TO_ADDR_LINE6>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_6));
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].addr_7, '\0', tt0007_ADDR_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].addr_7, sz_recvbufcat + gbp->count, tt0007_ADDR_LEN);
                ap_rprintf(r,"			<SHIP_TO_ADDR_LINE7>%s</SHIP_TO_ADDR_LINE7>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].addr_7));
                gbp->count += tt0007_ADDR_LEN;

                memset(ptr_recvbuf->level[gbp->i].line_item_num, '\0', tt0007_LINE_ITM_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].line_item_num, sz_recvbufcat + gbp->count, tt0007_LINE_ITM_LEN);
                ap_rprintf(r,"			<NUM_OF_LINE_ITEMS>%s</NUM_OF_LINE_ITEMS>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].line_item_num));
                gbp->count += tt0007_LINE_ITM_LEN;

                memset(ptr_recvbuf->level[gbp->i].status, '\0', tt0007_STATUS_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].status, sz_recvbufcat + gbp->count, tt0007_STATUS_LEN);
                ap_rprintf(r,"			<STATUS_OF_ORD_LEVEL>%s</STATUS_OF_ORD_LEVEL>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].status));
                gbp->count += tt0007_STATUS_LEN;

                memset(ptr_recvbuf->level[gbp->i].ship_method, '\0', tt0007_SHIP_METH_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].ship_method, sz_recvbufcat + gbp->count, tt0007_SHIP_METH_LEN);
                ap_rprintf(r,"			<ORD_LEVEL_SHIP_METHOD>%s</ORD_LEVEL_SHIP_METHOD>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].ship_method));
                gbp->count += tt0007_SHIP_METH_LEN;

                memset(ptr_recvbuf->level[gbp->i].ship_method_type, '\0', tt0007_SM_TYPE_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].ship_method_type, sz_recvbufcat + gbp->count, tt0007_SM_TYPE_LEN);
                ap_rprintf(r,"			<SHIP_METHOD_TYPE>%s</SHIP_METHOD_TYPE>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].ship_method_type));
                gbp->count += tt0007_SM_TYPE_LEN;
        
                memset(ptr_recvbuf->level[gbp->i].ship_date, '\0', tt0007_SHIP_DATE_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].ship_date, sz_recvbufcat + gbp->count, tt0007_SHIP_DATE_LEN);
                ap_rprintf(r,"			<LAST_SHIP_DATE>%s</LAST_SHIP_DATE>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].ship_date));
                gbp->count += tt0007_SHIP_DATE_LEN;

                memset(ptr_recvbuf->level[gbp->i].track_num, '\0', tt0007_TRACK_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].track_num, sz_recvbufcat + gbp->count, tt0007_TRACK_LEN);
                ap_rprintf(r,"			<TRACKING_NUM>%s</TRACKING_NUM>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].track_num));
                gbp->count += tt0007_TRACK_LEN;

                memset(ptr_recvbuf->level[gbp->i].suffix, '\0', tt0007_SUFFIX_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].suffix, sz_recvbufcat + gbp->count, tt0007_SUFFIX_LEN);
                ap_rprintf(r,"			<ORD_LEVEL_SUFFIX>%s</ORD_LEVEL_SUFFIX>\n", handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].suffix));
                gbp->count += tt0007_SUFFIX_LEN;

                memset(ptr_recvbuf->level[gbp->i].filler2, '\0', tt0007_FILLER2_LEN + 1);
                memcpy(ptr_recvbuf->level[gbp->i].filler2, sz_recvbufcat + gbp->count, tt0007_FILLER2_LEN);
                ...
                {
                        memset(ptr_recvbuf->level[gbp->i].item[gbp->k].item_num, '\0', tt0007_ITM_NO_LEN + 1);
                        memcpy(ptr_recvbuf->level[gbp->i].item[gbp->k].item_num, sz_recvbufcat + gbp->count, tt0007_ITM_NO_LEN);
                        gbp->count += tt0007_ITM_NO_LEN;

                        memset(ptr_recvbuf->level[gbp->i].item[gbp->k].item_desc, '\0', tt0007_ITM_DESC_LEN + 1);
                        memcpy(ptr_recvbuf->level[gbp->i].item[gbp->k].item_desc, sz_recvbufcat + gbp->count, tt0007_ITM_DESC_LEN);
                        gbp->count += tt0007_ITM_DESC_LEN;

                        memset(ptr_recvbuf->level[gbp->i].item[gbp->k].item_status, '\0', tt0007_ITM_STATUS_LEN + 1);
                        memcpy(ptr_recvbuf->level[gbp->i].item[gbp->k].item_status, sz_recvbufcat + gbp->count, tt0007_ITM_STATUS_LEN);
                        gbp->count += tt0007_ITM_STATUS_LEN;

                        memset(ptr_recvbuf->level[gbp->i].item[gbp->k].item_qty, '\0', tt0007_ITM_QTY_LEN + 1);
                        memcpy(ptr_recvbuf->level[gbp->i].item[gbp->k].item_qty, sz_recvbufcat + gbp->count, tt0007_ITM_QTY_LEN);
                        gbp->count += tt0007_ITM_QTY_LEN;

                        memset(ptr_recvbuf->level[gbp->i].item[gbp->k].item_price, '\0', tt0007_ITM_PRICE_LEN + 1);
                        memcpy(ptr_recvbuf->level[gbp->i].item[gbp->k].item_price, sz_recvbufcat + gbp->count, tt0007_ITM_PRICE_LEN);
                        gbp->count += tt0007_ITM_PRICE_LEN;

                        memset(ptr_recvbuf->level[gbp->i].item[gbp->k].item_gift_cert, '\0', tt0007_GFT_CERT_LEN + 1);
                        memcpy(ptr_recvbuf->level[gbp->i].item[gbp->k].item_gift_cert, sz_recvbufcat + gbp->count, tt0007_GFT_CERT_LEN);
                        gbp->count += tt0007_GFT_CERT_LEN;
                }

                gbp->j = atoi(ptr_recvbuf->level[gbp->i].line_item_num);
                ...
                for(gbp->k = 0; gbp->k < gbp->j; gbp->k++) 
                {
                     ...
                     ap_rprintf(r,"			<LINE_ITEM_PROD_NUM>%s</LINE_ITEM_PROD_NUM>\n", 
                                  handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].item[gbp->k].item_num));
                     ap_rprintf(r,"			<LINE_ITEM_PROD_DESCRIPTION>%s</LINE_ITEM_PROD_DESCRIPTION>\n", 
                                  handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].item[gbp->k].item_desc));
                     ap_rprintf(r,"			<LINE_ITEM_STATUS>%s</LINE_ITEM_STATUS>\n", 
                                  handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].item[gbp->k].item_status));
                     ap_rprintf(r,"			<LINE_ITEM_SALEABLE_QTY>%s</LINE_ITEM_SALEABLE_QTY>\n", 
                                  handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].item[gbp->k].item_qty));
                     ap_rprintf(r,"			<LINE_ITEM_PROD_PRICE>%s</LINE_ITEM_PROD_PRICE>\n", 
                                  handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].item[gbp->k].item_price));
                     ap_rprintf(r,"			<LINE_ITEM_GIFT_CERT>%s</LINE_ITEM_GIFT_CERT>\n", 
                                  handle_special_chars(gbp,ptr_recvbuf->level[gbp->i].item[gbp->k].item_gift_cert));
                     ap_rprintf(r,"		<LINE_ITEM_DETAILS>\n");
                }
