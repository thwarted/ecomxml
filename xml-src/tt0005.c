...
int tt0005_lt_process(request_rec *r, struct global_struct *gbp, char *stdout_buffer)
{
    ...
    strcpy(gbp->sendbuf0005->record_id, "0005");
        ...
        get_tag_data("ENCRYPT_CUST_EDP", gbp->sendbuf0005->encrypt_cust_edp,gbp,stdout_buffer);
        get_tag_data("PAGE_NUM", gbp->sendbuf0005->page_num,gbp,stdout_buffer);
        get_tag_data("START_DATE", gbp->sendbuf0005->start_date,gbp,stdout_buffer);
        get_tag_data("END_DATE", gbp->sendbuf0005->end_date,gbp,stdout_buffer);
...
int tt0005_CatSendStr(struct global_struct *gbp, char *sz_sendbufcat, tt0005_st_send *ptr_sendbuf)
{
    gbp->j = sprintf(sz_sendbufcat,
            "%-4s%-4s%-2s%-2s%-16.16s%-16.16s%-25.25s%-17.17s%04d%-10s%-10s",
            ...
            ptr_sendbuf->encrypt_cust_edp,
                        ...
                        ptr_sendbuf->start_date,
                        ptr_sendbuf->end_date);
...
*/
    ...
    for(gbp->i = 0; gbp->i < gbp->k; gbp->i++) 
        ...
        gbp->count += tt0005_TOT_ORDERS_LEN;

        memset(ptr_recvbuf->tot_dollars, '\0', tt0005_TOT_DOLLARS_LEN + 1);
        memcpy(ptr_recvbuf->tot_dollars,         sz_recvbufcat + gbp->count, tt0005_TOT_DOLLARS_LEN);
        gbp->count += tt0005_TOT_DOLLARS_LEN;
